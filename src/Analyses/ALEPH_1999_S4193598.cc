...
namespace Rivet {
  ...
  namespace {
    ...
    bool hasDecayedTo(const GenParticle* p, int id1, int id2) {
      bool decision = false;
      const GenVertex* decV  = p->end_vertex();
      std::vector<int> decids;
      if (decV->particles_out_size() == 2) {
        for (GenVertex::particles_out_const_iterator pp = decV->particles_out_const_begin(); pp != decV->particles_out_const_end() ; ++pp) {
