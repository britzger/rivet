...
namespace Rivet {
  ...
  public:
    ...
    void init() {
      ...
      vector<pair<double, double> > etaRanges_EL;
      etaRanges_EL.push_back(make_pair(-1.37, 1.37));
      etaRanges_EL.push_back(make_pair(-2.47, -1.52));
      etaRanges_EL.push_back(make_pair(1.52, 2.47));

      vector<pair<double, double> > etaRanges_MU;
      etaRanges_MU.push_back(make_pair(-2.4, 2.4));
      ...
      //
      //////////////////////////////////////////////////////////
      DressedLeptons electronFS(Photon, bare_EL, 0.1, true, etaRanges_EL, 20, false);
      addProjection(electronFS, "ELECTRON_FS");

      DressedLeptons muonFS(Photon, bare_MU, 0.1, true, etaRanges_MU, 20, false);
