...
namespace Rivet {
                                                          ...
                                                          const string& name)
  {
    ...
    if (!dupOk) {
      cerr << "Duplicate name '" << name << "' in parent '" << parent.name() << "'." << '\n';
      exit(1);
    }
  ...
  unique_ptr<Projection> ProjectionHandler::_clone(const Projection& proj)
  {
    ...
    getLog() << Log::TRACE << "Cloning projection " << proj.name() << " from " << &proj << "..." << '\n';
    unique_ptr<Projection> newproj = proj.clone();
    getLog() << Log::TRACE << "...cloned to " << proj.name() << " at " << newproj.get() << '\n';
                                                 ...
                                                 const string& name)
  {
    ...
    getLog() << Log::TRACE << "Registering new projection at " << p.get()
      << ". Starting refcount: " << p.use_count() << '\n';

    // Add the passed Projection to _projs

    _projs.insert(p);
    getLog() << Log::TRACE
      << "** inserted " << p.get() << " to lookup. Refcount: " << p.use_count() << '\n';
  ...
  ProjHandle ProjectionHandler::_getEquiv(const Projection& proj) const
  {
    ...
    getLog() << Log::TRACE << "RTTI type of " << &proj << " is " << newtype.name() << '\n';

    // Compare to ALL projections via _projs collection
    getLog() << Log::TRACE << "Comparing " << &proj
             << " with " << _projs.size()
             << " registered projection" << (_projs.size() == 1 ? "" : "s") <<  '\n';
    for (const ProjHandle& ph : _projs) {
      ...
      if (pcmp(*ph, proj) != CmpState::EQ) {
        ...
        getLog() << Log::TRACE << "  MATCH! Projections at "
                 << &proj << " and " << ph << " are equivalent" << '\n';
        return ph;
      }
    }
    getLog() << Log::TRACE << "  Nothing matches." << '\n';
  ...
  string ProjectionHandler::_getStatus() const {
    ...
    msg << "Current projection hierarchy:" << '\n';
