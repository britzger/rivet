...
namespace Rivet {
  ...
  const BinnedHistogram<T>& BinnedHistogram<T>::addHistogram(const T& binMin,
    ...
    bool found = false;
    foreach (YODA::Histo1DPtr hist, _histos) {
      if (hist == histo) {
        found = true;
        break;
      }
  ...
  template<typename T>
  YODA::Histo1DPtr BinnedHistogram<T>::fill(const T& bin,
                                                     const T& val,
                                                     double weight) {

    typename map<T, YODA::Histo1DPtr>::iterator histIt =
      _histosByUpperBound.upper_bound(bin);
    //check that the bin is not out of range
    if (histIt == _histosByUpperBound.end()) {
      return YODA::Histo1DPtr();
    }

    YODA::Histo1DPtr histo = histIt->second;
  ...
  void BinnedHistogram<T>::scale(const T& scale, Analysis* ana) {
    foreach (YODA::Histo1DPtr hist, getHistograms()) {
