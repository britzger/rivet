...
namespace Rivet {
  ...
  const Histo1DPtr BinnedHistogram<T>::histo(const T& binval) const {
    ...
    if (histIt1 == _histosByUpperBound.end()) return Histo1DPtr();
    ...
    if (histIt2 == _histosByLowerBound.begin()) return Histo1DPtr();
    ...
    --histIt2;
    if (histo != histIt2->second) return Histo1DPtr();
  ...
  template<typename T>
  Histo1DPtr BinnedHistogram<T>::fill(const T& binval, double val, double weight) {
