...
std::ostream& Rivet::operator<< (std::ostream& s, const Vec4D& vec)
{
  return s<<'('<<vec[0]<<','<<vec[1]<<','<<vec[2]<<','<<vec[3]<<')';
}

std::ostream& Rivet::operator<< (std::ostream& s, const Vec3D& vec)
{
  return s<<'('<<vec[1]<<','<<vec[2]<<','<<vec[3]<<')';
}

/*
std::istream& Rivet::operator>>(std::istream& s,Vec4D& vec)
{
  std::string out;
  s>>out;
...
std::istream& Rivet::operator>>(std::istream& s,Vec3D& vec)
{
  std::string out;
  s>>out;
  if (out.length()==0 || out[0]!='(' || out[out.length()-1]!=')')
...
bool Rivet::operator==(const Vec4D& v1, const Vec4D& v2) 
{
...
{
  for(short unsigned int i(0);i<4;++i) 
    if (!Rivet::IsZero(m_x[i])) return false;
  return true;
}
...
{ 
  Vec3D perp=1./Rivet::Max(Vec3D(ref).Abs(),1.e-12)*Vec3D(ref);
...
bool Rivet::operator==(const Vec3D& v1, const Vec3D& v2) 
{
