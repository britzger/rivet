...
namespace Rivet {
  ...
  public:
    ...
    void init() {
      ...
      _histLeps      = bookHistogram1D("LepMult", "Total charged multiplicity", 250, -0.5, 499.5);
      _histNotLeps   = bookHistogram1D("NotLepMult", "Total charged multiplicity", 250, -0.5, 499.5);

      _histChTot     = bookHistogram1D("TotalChMult", "Total charged multiplicity", 250, -0.5, 499.5);
      _histChLeps    = bookHistogram1D("LepChMult", "Total charged multiplicity", 250, -0.5, 499.5);
      _histChNotLeps = bookHistogram1D("NotLepChMult", "Total charged multiplicity", 250, -0.5, 499.5);
    }
    ...
    void analyze(const Event& event) {
      ...
      foreach (const Particle& p, fsparts) {
        if (PID::isLepton(p.pdgId())) lepmult += 1;
      }
      ...
      // Fill charged particle histograms
      {
        ...
        foreach (const Particle& p, cfsparts) {
          if (PID::isLepton(p.pdgId())) chlepmult += 1;
        }
    ...
    void finalize() { 
      ...
      normalize(_histLeps);
      normalize(_histNotLeps);
      normalize(_histChLeps);
      normalize(_histChNotLeps);
    }
