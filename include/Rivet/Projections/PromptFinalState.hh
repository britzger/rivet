...
namespace Rivet {
  ...
  public:
    ...
    PromptFinalState(double mineta=-MAXDOUBLE, double maxeta=MAXDOUBLE, double minpt=0.0*GeV);

    /// A constructor which allows to specify multiple eta ranges
    /// and the min \f$ p_T \f$.
    PromptFinalState(const vector<pair<double, double> >& etaRanges, double minpt=0.0*GeV);
