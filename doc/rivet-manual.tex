...
\newenvironment{philosophy}{\vspace{5mm}\hrule\vspace{3mm}\begingroup\it\begin{window}[0,l,{\bendimg},{}]}{%
  \end{window}\endgroup\vspace{3mm}\hrule\vspace{5mm}\ignorespacesafterend}
...
the parent class, so you need not worry about name clashes. A very important
point is that the passed \code{Projection} must have a lifetime that extends
beyond the scope of the constructor in which \code{addProjection} is called: the
projection handler may delete projections, but it cannot construct
them.\footnote{We wish it could\dots} typically, this mean that it will be
\code{new}ed as it is passed, e.g. \code{addProjection(*new MyProjection(\dots),
  "MyName");}.

\begin{philosophy}
  At this point, if you have bought into C++y ideas like super-strong
  ...
  legal. These are very legitimate thoughts!
  ...
  projection lifetime and class slicing. At least when this goes wrong it will
  throw an unmissable \emph{runtime} error every time that you run it (until
  it's fixed, of course!) rather than silently do the wrong thing.

  We take some solace in the fact that very good programs have been and are
  ...
  As a final thought, one reasin for Rivet's internal complexity is that C++ is
  ...
  interpreted analysis code: I certainly hope so! For now, though, you will have
  to be content with ``slightly less safe C++''!
\end{philosophy}
