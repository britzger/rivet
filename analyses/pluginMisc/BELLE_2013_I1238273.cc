...
namespace Rivet {
  ...
  public:
    ...
    void analyze(const Event& event) {
      ...
      for(const Particle& p : apply<UnstableFinalState>(event, "UFS").particles(Cuts::pid==PID::B0BAR)) {
      ...
      // Loop over B- Mesons
      for(const Particle& p : apply<UnstableFinalState>(event, "UFS").particles(Cuts::pid==PID::BMINUS)) {
