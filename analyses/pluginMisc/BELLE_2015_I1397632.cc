...
namespace Rivet {
  ...
  public:
    ...
    void analyze(const Event& event) {
      // Get B0 Mesons
      for(const Particle& p : apply<UnstableFinalState>(event, "UFS").particles(Cuts::pid==PID::B0)) {
      ...
      // Get B+ Mesons
      for(const Particle& p : apply<UnstableFinalState>(event, "UFS").particles(Cuts::pid==PID::BPLUS)) {
