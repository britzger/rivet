...
namespace Rivet {
  ...
  public:
    ...
    void init() {
      ...
      TauFinder taus(TauFinder::LEPTONIC);
      declare(taus, "Taus");
      ...
      for (size_t i = 0; i < 3; ++i) {
        for (size_t j = 0; j < 4; ++j)
          _srcounts_ewino[i][j] = bookCounter("sr_ewino_" + toString(i) + "_" + toString(j));
        for (size_t j = 0; j < 3; ++j)
          _srcounts_stop[i][j] = bookCounter("sr_stop_" + toString(i) + "_" + toString(j));
      }
    ...
    void analyze(const Event& event) {
      ...
      if (sameflav) {
        ...
        _srcounts_ewino[iptm][imll < 1 ? 0 : imll-1]->fill(weight);

      } else {

        // Stop SR
        if (leptons[0].abspid() == PID::MUON and leptons[0].pT() < 5*GeV) vetoEvent;
        _cutflows["St"].fill(13, weight);
        ...
        _srcounts_stop[iptm][iptl]->fill(weight);
      }
