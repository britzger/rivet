...
namespace Rivet {
  ...
  public:
    ...
    void init() {
      ...
      // Book histograms
      _h_sigmaM = bookHisto1D("sigmaM",50, 200, 2000);
      _h_modeta = bookHisto1D("ModEta12", 42, 0, 4.2);
    ...
    void analyze(const Event& event) {
      ...
      _h_sigmaM->fill(sigmaM, weight);

      // Build deta between two leading large-R jets
      const double delta_eta = fabs(deltaEta(LRJ[0], LRJ[1]));
      _h_modeta->fill(delta_eta, weight);

      // CutFlow1
      if (LRJ.size() >= 4) {
        _flows["CutFlow1"].fill(1, weight);
        if (delta_eta < 1.4) {
          _flows["CutFlow1"].fill(2, weight);
          if (LRJ[0].pT() > 400*GeV) {
            _flows["CutFlow1"].fill(3, weight);
            if (sigmaM > 1000*GeV)
              _flows["CutFlow1"].fill(4, weight);
            if (tagg == 0) {
              _flows["CutFlow1"].fill(5, weight);
              if (sigmaM > 1000*GeV){
                _flows["CutFlow1"].fill(6, weight);
              }
            } //end of btagg loop
          } //end of pT loop
        } //end of delta loop
      ...
      // CutFlow2
      if (LRJ.size() >= 4) {
        _flows["CutFlow2"].fill(1, weight);
        if (delta_eta < 1.4) {
          _flows["CutFlow2"].fill(2, weight);
          if (LRJ.size() >= 5) {
            _flows["CutFlow2"].fill(3, weight);
            if (sigmaM > 800*GeV)
              _flows["CutFlow2"].fill(4, weight);
            if (tagg == 0) {
              _flows["CutFlow2"].fill(5, weight);
              if (sigmaM > 800*GeV){_flows["CutFlow2"].fill(6, weight);
              }
            } //end of btagg loop
          } //Njet>5 loop
        } //end of delta loop
