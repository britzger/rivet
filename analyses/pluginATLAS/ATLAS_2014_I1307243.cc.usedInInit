...
namespace Rivet {
    ...
    void init() {
      ...
      foreach (const string& gapCategory, _gapCategories ) {
        ...
        for (size_t dyLow = 0; dyLow < _dy_max; ++dyLow ) {
          ...
          _h_tmp_events_Q0_dySlices[gapCategory].add( dyLow, dyLow+1, _h_tmp_events_Q0_single_dySlice );
          Histo1DPtr tmp;
          _h_crossSection_dphi_dySlices[gapCategory].add( dyLow, dyLow+1, book(tmp, 13+(_dy_max*gapCategoryOffset)+dyLow, 1, 1));
        }
  ...
  private:
    ...
    map<string, BinnedHistogram > _h_tmp_events_Q0_dySlices;
    ...
    map<string, BinnedHistogram > _h_crossSection_dphi_dySlices;
  };
