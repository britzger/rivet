...
namespace Rivet {
  ...
  public:
    ...
    void init() {
      ...
      FastJets fj04(fs, FastJets::ANTIKT, 0.4, JetAlg::ALL_MUONS, JetAlg::ALL_INVISIBLES);
      ...
      for(size_t i=0;i<nybins;++i){// loop over |y| bins
        _pThistograms.addHistogram(ybins[i], ybins[i+1], bookHisto1D(i+1, 1, 1));
      }
      // mjj histograms
      for(size_t i=0;i<nybins;++i){// loop over ystar bins
        _mjjhistograms.addHistogram(ystarbins[i], ystarbins[i+1], bookHisto1D(i+7, 1, 1));
      }
    ...
    void analyze(const Event& event) {
      ...
      for(int ijet=0;ijet<nJets;++ijet){ // loop over jets
        ...
        if(jet.pt()>100.0*GeV){
          ...
          _pThistograms.fill(absy,jet.pt()/GeV, weight);
        }
      }
      ...
      if(nJets > 1){ // skip events with less than 2 jets passing pT>75GeV and |y|<3.0 cuts
        ...
        if(HT2>200*GeV && ystar<3.0){
          // Fill distribution
          _mjjhistograms.fill(ystar,mass/GeV, weight);
        }
      }
  ...
  private:
    ...
    BinnedHistogram<double> _pThistograms;
    // The dijet mass spectrum for akt4 jets
    BinnedHistogram<double> _mjjhistograms;
