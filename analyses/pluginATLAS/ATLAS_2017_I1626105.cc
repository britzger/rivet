...
namespace Rivet {
  ...
  public:
    ...
    void analyze(const Event& event) {
      ...
      vector<DressedLepton> elpass;
      vector<DressedLepton> mupass;
      for (const Jet& jet : jets) {
        for (const DressedLepton& el : elecs) {
          if (deltaR(jet, el) >= 0.4) elpass.push_back(el);
        }
        for (const DressedLepton& mu : muons) {
          if (deltaR(jet, mu) >= 0.4) mupass.push_back(mu);
        }
      }
      if (elpass.empty() || mupass.empty())  vetoEvent;
      elecs = elpass;
      muons = mupass;
