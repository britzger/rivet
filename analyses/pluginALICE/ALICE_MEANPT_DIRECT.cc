...
namespace Rivet {
  ...
  class ALICE_MEANPT_DIRECT : public Analysis {
  public:

    /// Constructor
    DEFAULT_RIVET_ANALYSIS_CTOR(ALICE_MEANPT_DIRECT);
    ...
    void init() {
      ...
      declare(PrimaryResonances(Cuts::absrap < 0.5),"PRy");
    ...
    void analyze(const Event& event) {
      if (apply<PrimaryParticles>(event,"PP").particles().size() < 1) vetoEvent;
      const PrimaryParticles& prim = apply<PrimaryParticles>(event,"PPy");
      const PrimaryResonances& primR = apply<PrimaryResonances>(event,"PRy");
  ...
  DECLARE_RIVET_PLUGIN(ALICE_MEANPT_DIRECT);
