...
namespace Rivet {
  ...
  public:
	...
	for (int ipt = 0; ipt < PT_BINS; ipt++) {
	  ...
	  book(_histYield[itype][ipt], "Yield_" + event_string[itype]
      ...
      book(_histYieldBkgRemoved[itype][ipt], "Yield_" +
      ...
      book(_histTriggerCounter, "Trigger", EVENT_TYPES, 0.0,
	...
	for (int ipt = 0; ipt < PT_BINS; ipt++) {
	  ...
	  for (const auto& associatedParticle : associatedParticles[ipt]) {
		...
		_histYield[event_type][ipt]->fill(dPhi);
	      }
	    }
	  }
	}
