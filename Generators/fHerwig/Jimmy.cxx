// ------------------------------------------------------------- 
// File:Jimmy.cxx
// Generators/Jimmy.cxx Description: Allows the user
// to generate Jimmy events and store the result in the
// Transient Store.
//
// AuthorList:
...
std::string	jimmy_stream	=	"JIMMY_INIT";
extern "C" double atl_hwrgen_( int* idummy )
{
  CLHEP::HepRandomEngine* engine = p_AtRndmGenSvc->GetEngine(jimmy_stream);
...
Atlas_HEPEVT*  Jimmy::atlas_HEPEVT = new Atlas_HEPEVT();

//--------------------------------------------------------------------------
Jimmy::Jimmy(const std::string& name, 
	     ISvcLocator* pSvcLocator): GenModule(name,pSvcLocator)
{
  //--------------------------------------------------------------------------  
  declareProperty("JimmyCommand", m_jimmyCommandVector);
}
//--------------------------------------------------------------------------
Jimmy::~Jimmy(){
  //--------------------------------------------------------------------------
...
StatusCode Jimmy::genInitialize() {
  ...
  CommandVector::const_iterator ic              =       m_jimmyCommandVector.begin();
  ...
  do {
    ...
    ic++;
  } while (ic != m_jimmyCommandVector.end() && proc_not_found);
  //  initialized  values of Localpars into Jimmy Commons
  Localpar2Her();
  // Call Fortran Routine to Initialize Jimmy Common Block
  // cout <<  " calling herwiginterface" << endl;
  ic              =       m_jimmyCommandVector.begin();
  ...
  do {
    ...
    ic++;
  } while (ic != m_jimmyCommandVector.end() && itopd_not_found);
  herwiginterface_(&m_itopd);
  // Transfer numbers from Jimmy Common Blocks to Localpars
  //   cout <<  " calling Her2Localpar" << endl;
  Her2Localpar();  
  //now read in all the parameter changes from the user
  for (size_t i = 0; i < m_jimmyCommandVector.size(); i++) {
    log << MSG:: INFO  << " Command is: " << m_jimmyCommandVector[i] << endreq;
    StringParse mystring(m_jimmyCommandVector[i]);
    string myvar=mystring.piece(1);
    ...
    // Jimmy common blocks
    else if(myvar=="msflag"){m_msflag=myint1;}
    else if(myvar=="jmbug"){m_jmbug=myint1;}
    else if(myvar=="ptjim"){m_ptjim= (double) myfl1;}
    else if(myvar=="jmueo"){m_jmueo=  myint1;}
    else if(myvar=="jmrad"){
      if (myint1 > 0 && myint1 < 264) { gJmparm -> jmrad[myint1-1] = myfl2; }
      else {  log << MSG:: ERROR << " JMRAD ARRAY HAS ONLY 264 elements [1 to 264]" << endreq; }
    } 
    else {
      if (myvar!="topdec") log << MSG:: INFO << " ERROR in JIMMY PARAMETERS   " << myvar << " is not a variable name that I recognize. it could be a typo on your part or a mistake on mine. Not all variables are changeable" << endreq;
    }
  }
  ...
  // Save the JIMMY_INIT stream seeds....
  CLHEP::HepRandomEngine* engine = p_AtRndmGenSvc->GetEngine(jimmy_stream);
  ...
  Localpar2Her();
  // now for susy
  if((m_process >= 3000 && m_process < 5000) || (m_process >= 13000 && m_process < 15000)){
    int syunit = 66;
  ...
  p_AtRndmGenSvc->CreateStream(si1, si2, jimmy_stream);
  jimmy_stream = "JIMMY";
...
//---------------------------------------------------------------------------
StatusCode Jimmy::callGenerator() {
  //---------------------------------------------------------------------------
  MsgStream log(messageService(), name());
  log << MSG:: INFO << " JIMMY generating.  \n"  << endreq;
  bool goodev = false;
  int abort = 1;
  ...
  while(!goodev && abort==1){
    ...
    if (abort == 0) {
      ...
      int ihwcod = 1;
      hwanal_(&ihwcod);
      //    std::cout << "gHwevnt->ierror " << gHwevnt->ierror << std::endl;
      if(gHwevnt->ierror == 0){
	goodev=true;
      } else { //modified by T.Sasaki
	goodev=false; //explicit , not necessary;
	abort=1; //for loop again
      }
    }
  }
  return StatusCode::SUCCESS;  
  log << MSG:: INFO << " JIMMY generating done.  \n"  << endreq;
}

//---------------------------------------------------------------------------
StatusCode Jimmy::genFinalize() {
  //---------------------------------------------------------------------------
  MsgStream log(messageService(), name());
  log << MSG:: INFO << " JIMMY Ending.  \n"  << endreq;
...
//---------------------------------------------------------------------------
StatusCode Jimmy::fillEvt(GenEvent* evt) {
  //---------------------------------------------------------------------------
  MsgStream log(messageService(), name());

  log << MSG::INFO << " JIMMY Atlas_HEPEVT Filing.  \n"  << endreq;
  store_Atlas_HEPEVT();

  log << MSG:: INFO << " JIMMY Filing.  \n"  << endreq;
  ...
  //  std::cout << " jimmy thing " << gJmparm -> jmueo<< std::endl ;
  hepio.fill_next_event(evt);
  int pr_id = HERWIG + 100000 * m_ExternalProcess + m_process%10000;
...
void Jimmy::Her2Localpar() {
  ...
  // Jimmy common blocks
  m_msflag=gJmparm -> msflag;
  m_jmbug=gJmparm -> jmbug;
  m_ptjim=gJmparm -> ptjim;
  m_jmueo=gJmparm -> jmueo;
...
//
// This routine takes the Localpars of the Herwig Module and transfers them
// to the Herwig Common Blocks

void Jimmy::Localpar2Her() {
  int nch;
  ...
  // Jimmy common blocks
  gJmparm -> msflag = m_msflag;
  gJmparm -> jmbug = m_jmbug;
  gJmparm -> ptjim = m_ptjim;
  gJmparm -> jmueo = m_jmueo;

}

void
Jimmy::store_Atlas_HEPEVT(void)
{
  MsgStream log(messageService(), name());
