// --------------------------------------------------
// 
// File:  Generators/Jimmy.h 
// Description:
//    This code is used to get a JIMMY Monte Carlo event.
...
#ifndef GENERATORMODULESJIMMY_H
#define GENERATORMODULESJIMMY_H
...
{
public:
  Jimmy(const std::string& name, ISvcLocator* pSvcLocator);
  virtual ~Jimmy();
...
private:
  // data members corresponding to Jimmy variables
...
//cjl  double m_rapiditiesm_Parm;// select rapidity limits min
  ...
  int m_iprint_Parm;// Jimmy print level control
  int m_maxpr_Parm;// Dump of Jimmy events
  ...
  int m_syspin_Parm;// Jimmy syspin control
  int m_spcopt_Parm;
  ...
  int m_jmueo;
    ...
    // Commands to setup jimmy from jobOptions
  CommandVector m_jimmyCommandVector;
